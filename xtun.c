...
typedef struct xtun_path_s {
    ...
    u32 seila;
    u16 reserved;
    u16 bandSrv; // BANDA DO SERVIDOR, A REPASSAR A ELE; INTERVALO DE ENVIO DISSO/AUTH
#endif
    u32 band;
    u16 reserved;
...
static rx_handler_result_t xtun_in (sk_buff_s** const pskb) {
    ...
    const xtun_path_s* const hdr = PTR(skb_mac_header(skb)) - XTUN_SIZE_PRIVATE;
...
static int __init xtun_init(void) {
    ...
    BUILD_BUG_ON(sizeof(xtun_path_s) != XTUN_SIZE);
    BUILD_BUG_ON(sizeof(xtun_path_s) != XTUN_SIZE_ALL);
    ...
    for (uint nid = 0; nid != NODES_N; nid++) {
        ...
        for (uint pid = 0; pid != PATHS_N; pid++) {
            ...
            printk("XTUN: TUNNEL %s: NODE #%u INITIALIZING WITH BAND %u SECRET 0x%04X PHYS %s TOS 0x%02X TTL %u\n"
                " CLT MAC %02X:%02X:%02X:%02X:%02X:%02X IP %u.%u.%u.%u PORT %u\n"
                " SRV MAC %02X:%02X:%02X:%02X:%02X:%02X IP %u.%u.%u.%u PORT %u\n",
                cfgNode->virt, nid, cfgPath->band, cfgNode->secret, cfgPath->phys, cfgPath->iTOS, cfgPath->iTTL,
            ...
            xtun->eSrc[2]    =  BE16(0);
#else
            xtun->eDst[0]    =  BE16(cfgPath->srvMAC16[0]);
            xtun->eDst[1]    =  BE16(cfgPath->srvMAC16[1]);
            xtun->eDst[2]    =  BE16(cfgPath->srvMAC16[2]);
            xtun->eSrc[0]    =  BE16(cfgPath->cltMAC16[0]);
            xtun->eSrc[1]    =  BE16(cfgPath->cltMAC16[1]);
            xtun->eSrc[2]    =  BE16(cfgPath->cltMAC16[2]);
#endif
            xtun->eType      =  BE16(ETH_P_IP); // FIXED
            xtun->iVersion   =  BE8(0x45); // FIXED
            xtun->iTOS       =  BE8(cfgPath->iTOS); // MAY BE ALTERED IN TRANSIT
            ...
            xtun->iTTL       =  BE8(cfgPath->iTTL); // MAY BE ALTERED IN TRANSIT
            ...
            xtun->uSrc       =  BE16(PORT(nid, pid));
            xtun->uDst       =  BE16(0); // WILL BE DISCOVERED ON INPUT
#else
            xtun->iSrc       =  BE32(cfgPath->cltAddr32);
            xtun->iDst       =  BE32(cfgPath->srvAddr32);
            xtun->uSrc       =  BE16(cfgPath->cltPort);
            xtun->uDst       =  BE16(PORT(nid, pid));
#endif
        ...
        net_device_s* const virt = alloc_netdev(sizeof(xtun_node_s*), cfgNode->name, NET_NAME_USER, xtun_dev_setup);
