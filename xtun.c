...
#endif
    u16 band;
    u16 key; // DINAMICO, GERADO PELO CLIENTE
...
static rx_handler_result_t xtun_in (sk_buff_s** const pskb) {
    ...
    if (hdr->iHash) {
        // NOT AUTH
        if (xtun_decode(xtun->secret, xtun->key, payload, SKB_TAIL_PTR(skb) - payload) != hdr->iHash)
            // HASH MISMATCH
            goto drop;
    } else {
        // AUTH
        ...
        if (skb->len != (XTUN_SIZE_ETH + XTUN_AUTH_SIZE))
            // INVALID AUTH SIZE
            goto drop;
        const u16 key = xtun_auth_key_check(xtun->secret, payload);
        if (!key)
            // INCORRECT AUTH
            goto drop;
        // USA ELA
...
static int __init xtun_init(void) {
    ...
    for (uint i = 0; i != ARRAY_COUNT(cfgs); i++) {
        ...
        printk("XTUN: TUNNEL %s: ID #%u INITIALIZING WITH BAND %u SECRET 0x%04X PHYS %s TOS 0x%02X TTL %u\n"
            " CLT MAC %02X:%02X:%02X:%02X:%02X:%02X IP %u.%u.%u.%u PORT %u\n"
            " SRV MAC %02X:%02X:%02X:%02X:%02X:%02X IP %u.%u.%u.%u PORT %u\n",
            cfg->virt, cfg->id, cfg->band, cfg->secret, cfg->phys, cfg->iTOS, cfg->iTTL,
