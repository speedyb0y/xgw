...
static inline int xgw_epoll_ready (const int fd) {
...
static inline void xgw_epoll_del (const int fd) {
...
static inline void xgw_epoll_add (const int fd) {
...
static void xgw_epoll_register (const int fd) {
...
static void xgw_epoll_register_connecting (const int fd) {
...
static void xgw_epoll_register_connected (const int fd) {
...
static int xgw_conn_established_ (const int in, const int out) {

    if (xgw_epoll_ready(in)) {
        xgw_epoll_del(in);
...
static int xgw_conn_established (conn_s* const conn) {

    return // NOTE: USING & BECAUSE WE WANT TO FLUSH BOTH, AND ONLY THEN CONSIDER THE RESULTS
        xgw_conn_established_(conn->srv, conn->clt)
      & xgw_conn_established_(conn->clt, conn->srv)
      ;
}

static int xgw_conn_request (conn_s* const conn) {
...
static int xgw_conn_request2 (conn_s* const conn) {
    ...
    xgw_epoll_register_connecting(srv);
...
static int xgw_conn_connect (conn_s* const conn) {

    if (!xgw_epoll_ready(conn->srv))
        return WAIT;
    ...
    xgw_epoll_register_connected(conn->srv);
    ...
    return xgw_conn_established(conn);
}
...
int main (void) {
    ...
    do {
        ...
        // CONNECTIONS
        static const xgw_conn_f funcs [] = {
            /* CONN_DO_ESTABLISHED */ xgw_conn_established,
            /* CONN_DO_REQUEST     */ xgw_conn_request,
            /* CONN_DO_CONNECT     */ xgw_conn_connect,
            /* CONN_DO_REQUEST2    */ xgw_conn_request2,
        };
...
failed:
            xgw_epoll_del(conn->clt); if (conn->srv)
            xgw_epoll_del(conn->srv);
        ...
        if (eventsN == -1) {
            ...
            while (eventsN--)
                xgw_epoll_add(events[eventsN].data.fd);
            fastAgain = 1;
        } elif (fastAgain)
            fastAgain = 0;

        // SIGNALS
        if (xgw_epoll_ready(sfd)) {
            xgw_epoll_del(sfd);
