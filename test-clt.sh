...
# XGW SERVER -> XGW LAN
ip -4 rule add prioritu 3 table 3 from 172.16.${NODE_ID}.0
ip -4 rule add prioritu 3 table 3  to  172.16.${NODE_ID}.0/20
ip -4 rule add priority 5 table 5 iif xgw

# XGW LAN -> XGW SERVER
ip -4 rule add priority 5 table 5  to  172.16.${NODE_ID}.0
ip -4 rule add priority 5 table 5 from 172.16.${NODE_ID}.0/20
ip -4 rule add priority 5 table 5 oif xgw

# NOTE: SE VAMOS FAZER FORWARDING, ENTAO
# ip -4 rule add table 4 to 172.16.${NODE_ID}.0/24
# ip -4 route add table 4 dev LAN_INTERFACE src 172.16.${NODE_ID}.20 172.16.${NODE_I>
